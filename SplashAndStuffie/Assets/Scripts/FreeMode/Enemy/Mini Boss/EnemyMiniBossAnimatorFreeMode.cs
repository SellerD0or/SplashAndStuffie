...
public class EnemyMiniBossAnimatorFreeMode : EnemyAnimtatorFreeMode
{
   ...
   private void Shoot()
   {
    ...
    {
            Debug.LogError("RUUUN!" +_enemy.PlaceRow.IsLowPlace );
        if(_enemy.PlaceRow.IsLowPlace == false)
         ArmatureComponent.animation.FadeIn("run");
         else
         {
           Debug.Log("run_alt");
                      ArmatureComponent.animation.FadeIn("run_alt");
         }
    }
}
