...
public class PlayerAkiBulletFreeMode : MonoBehaviour
{
...
//        Debug.LogError(_startPosition);
   ...
   public IEnumerator CoolDown(float waitTime)
    ...
    {
        Player = player;  
                int rotation = player.PlaceRow.IsRight ? 0 : 180;
        float size = player.PlaceRow.IsRight ? transform.localScale.y : -transform.localScale.y;
