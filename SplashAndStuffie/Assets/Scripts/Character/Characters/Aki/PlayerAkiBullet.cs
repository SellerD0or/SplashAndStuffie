...
public class PlayerAkiBullet : PlayerBullet, IMoveableBullet
{
...
//        Debug.LogError(_startPosition);
   ...
   public IEnumerator CoolDown(float waitTime)
    ...
    {
        Player = player;  
        int rotation = Player.IPlayerMovement.IsTurned ? 0 : 180;
        float size = Player.IPlayerMovement.IsTurned ? transform.localScale.y : -transform.localScale.y;
      ...
      if (other.TryGetComponent<Enemy>(out Enemy enemy))
      {
         ...
         Player.Ability.ReloadAmount(0.5f);
